...
const exec=promisify(execFile),resources=resolve('runtime');await mkdir('.build',{recursive:true});const root=await mkdtemp(resolve('.build/change-wire-')),workspace=join(root,'workspace'),home=join(root,'dsh');await mkdir(workspace);
