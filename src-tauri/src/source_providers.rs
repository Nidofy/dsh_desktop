...
// Reviewed payload/catalog interfaces, pinned to the sealed source build.
...
pub fn verify_payload_hook(runtime:&Path)->Result<(),String>{
    ...
    if receipt["source"]["commit"]!="5e2879f0478ba9336128312e715dee7a9f56c3db"||receipt["source"]["version"]!="0.1.7-alpha.2"||receipt["status"]!="SOURCE_SMOKE_PASSED"{return Err("源码 payload hook 来源不匹配".into());}
    let path="dsh/node_modules/@deepseek-ai/dsh-llm-pi-ai/lib/index.js";
    let expected="c62064e899f5defcbd04164267ff048ac1d42bb8d7ffcb74811475682ab8c846";
...
pub fn build(root:&Path,runtime:&Path,catalog:&profiles::Catalog,selected:&profiles::Profile)->Result<Configuration,String>{
    ...
    for profile in &catalog.profiles {
        ...
        let (catalog_route,config)=rows.iter().find(|row|row["id"]=="llm-pi-ai").and_then(|row|row["config"]["providers"].as_object()).and_then(|p|p.iter().next()).ok_or("提供方配置缺失")?;
        let mut provider=config.clone();
        if catalog_route!="desktop-internal" {provider["catalogProvider"]=json!(catalog_route);}
...
#[cfg(test)]mod tests{use super::*;
 #[test]fn same_model_connections_keep_distinct_routes_and_private_keys(){
  ...
  assert!(built.providers.as_object().unwrap().values().all(|p|p.get("catalogProvider").is_none()));
  let mut preset_catalog=catalog.clone();preset_catalog.profiles[0].connection.builtin_provider=Some("deepseek".into());
  let inherited=build(&root,&repo.join("runtime"),&preset_catalog,&preset_catalog.profiles[0]).unwrap();
  assert_eq!(inherited.providers[format!("desktop-{}",items[0].id)]["catalogProvider"],"deepseek");
  assert!(inherited.providers[format!("desktop-{}",items[1].id)].get("catalogProvider").is_none());
