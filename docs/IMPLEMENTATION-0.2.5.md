...
本记录接续 [M1 基线与首轮验证](IMPLEMENTATION-M1.md)。M1 的 T1–T6 已完成，0.2.5-rc.1 隔离源码候选通过完整构建、最终 EXE 原生验收和独立目录离线 Gate；不代表真实内网或企业现场通过。用户于 2026-09-24 授权本地提交及后续里程碑自动提交；不推送、不发布。以下各节保留对应里程碑发生时的状态，旧的“待授权／未准入”不代表当前状态。
...
### S6：完整构建、最终 EXE 原生验收与独立离线交付（完成，2026-09-24）

最终目录为 `dist/DSHDesktop-0.2.5-rc.1-win-x64-20260924-105055-4abdcccc`，29,247 个文件，1,456,694,971 字节（不计自校验清单），离线 read/edit/pwsh、重启后历史和显式项目路径、错误 Key/URL/模型、许可证及整树检查均通过；实际目录 EXE 另以全新合成资料启动并正常退出。目录与受测 EXE/运行时指纹一致。见 `distribution-verification.json`、`staged-package.json` 与 `package.log`。首次同名无后缀目录因 D 盘测试 workspace 的 `grantWrite` Win32 5 被 Gate 拒绝，保留并标记 `NOT_ACCEPTED.txt`；用完整 Gate 同一用户临时测试根重跑，未改变产品权限或测试断言。包内文档保持封存前记录，仓库本记录及交付证据记录随后完成的目录验收。
