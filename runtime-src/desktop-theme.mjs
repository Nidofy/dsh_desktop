...
export function installDesktopTheme(ctx, home) {
  const read = () => appearance(readHarnessSetting(ctx,'ui-theme'));
  let saved, writing = Promise.resolve();
  const publish = () => {
    if (!readHarnessSetting(ctx,'ui-theme')) return;
  ...
  ctx.on('settings/document-updated', ns => { if (ns === 'ui-theme') publish(); });
